...
export function BeamMesh({ w0, EI, showOriginal, deformationScale }: BeamMeshProps) {
  const { deformedGeometry, originalGeometry, maxDeflection } = useMemo(() => {
    ...
    for (let i = 0; i <= segments; i++) {
      ...
      const y = calculateDeflection((i / segments) * beamLength, w0, EI) * deformationScale
    ...
    deformedGeo.setAttribute("position", new Float32BufferAttribute(positions, 3))
    ...
    originalGeo.setAttribute("position", new Float32BufferAttribute(originalPositions, 3))
    originalGeo.computeVertexNormals()

    return { deformedGeometry: deformedGeo, originalGeometry: originalGeo, maxDeflection: maxDef }
